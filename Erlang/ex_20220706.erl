...
multiple_query(FunL, DataL) when length(FunL) /= length(dataL) ->
    error("Function list and Data list have different lengths.");

multiple_query(FunL, DataL) ->
    master_server ! {slaves_request, {identity, self()}, {quantity, length(funL)}},
    receive 
        {slaves_id, SlavesPIDs} -> ok
    end,
    [   S ! {compute_request, {identity, self()}, F, D} || {S, F, D} <- lists:zip3(SlavesPIDs, FunL, DataL)],
    [   receive 
